...
{
    uint8_t repeat_rate     : 5; // 0x00 to 0x1F (30Hz to 2Hz)
...
static inline __attribute__((__always_inline__)) int __flush_input_buffer(void)
{
...
static inline __attribute__((__always_inline__)) int __wait_for_output_buffer(void)
{
...
#define FLUSH_INPUT_BUFFER()                                                \
    {                                                                       \
        if (__flush_input_buffer()) {                                       \
            printf("ps2: input buffer flush failed (%s:%d)\n",              \
                __FILE__, __LINE__);                                        \
        }                                                                   \
    }
...
#define WAIT_FOR_OUTPUT_BUFFER()                                            \
    {                                                                       \
        if (__wait_for_output_buffer()) {                                   \
            printf("ps2: output buffer wait failed (%s:%d)\n",              \
                __FILE__, __LINE__);                                        \
        }                                                                   \
    }
