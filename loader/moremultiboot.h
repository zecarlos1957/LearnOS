...
static void move_kernel(void) {
	...
	multiboot_header.num = header->e_shnum;
	multiboot_header.size = header->e_shentsize;
	multiboot_header.addr = header->e_shoff;
	multiboot_header.shndx = header->e_shstrndx;
	 
	print("e_shnum");print_hex(multiboot_header.size);print("\n");
	print("e_shentsize");print_hex(multiboot_header.size);print("\n");
	print("e_shoff");print_hex(multiboot_header.addr);print("\n");
	print("e_shstrndx");print_hex(multiboot_header.shndx);print("\n");
...
static void do_it(struct ata_device * _device) {
	...
	if (navigate(kernel_path)) 
	{
		...
		long offset = 0;
		for (int i = dir_entry->extent_start_LSB; 
		     i < dir_entry->extent_start_LSB + dir_entry->extent_length_LSB / 2048 + 1; 
		     ++i, offset += 2048)
		{
...
#define SECTORS 512
	...
	{
		print(module_dir);print("not found\n");
	}
	return;
}
#endif
